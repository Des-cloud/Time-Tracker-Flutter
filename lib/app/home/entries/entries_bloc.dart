...
class EntriesBloc {
  ...
  Stream<List<JobEntry>> get _allEntriesStream => Rx.combineLatest2(
        database.streamOfEntries(),
        database.jobsStream(),
        _entriesJobsCombiner,
      );

  static List<JobEntry> _entriesJobsCombiner(
      List<Entry> entries, List<Job> jobs) {
    return entries.map((entry) {
      final job = jobs.firstWhere(
        (job) => job.jobID == entry.jobID,
        orElse: () => null,
      );
      return JobEntry(entry: entry, job: job);
    }).toList();
  }
  ...
  static List<EntriesListTileModel> _createModels(List<JobEntry> allEntries) {
